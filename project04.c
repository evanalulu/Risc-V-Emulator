...
void get_bitseq_signed_test(int start_arg, char **argv, struct rv_state *state) {
    int r;
    int n = atoi(argv[start_arg]);
    int start = atoi(argv[start_arg + 1]);
    int end = atoi(argv[start_arg + 2]);
...
void countll_test(int start_arg, char **argv, struct rv_state *state) {
    // Make a copy of the string since we modify it in place
    struct node_st nodes[1024];
    int size = atoi(argv[start_arg]);
    int i, r;
    ...
    rv_init(state, (uint32_t*) countll_s, (uint64_t) nodes, 0, 0, 0);
    r = rv_emulate(state);
    printf("Emu: %d\n", r);
}
...
void swap_test(int start_arg, int argc, char **argv, struct rv_state *state) {
    int tmp[MAX_ARRAY];
    int a[MAX_ARRAY] = {0,};
    int len = 0;
    int i, j;
...
int main(int argc, char **argv) {
    ...
    } else if (!strcmp(argv[i], "countll")) {
        countll_test(i + 1, argv, &state);
